<template>
  <div class="consultas-container">
      ...
      <div v-else-if="consultas.length > 0" class="items-list">
        <div v-for="consulta in consultas" :key="consulta.id" class="item-card" @click="abrirModalDetalle(consulta)">
          <div class="item-content">
            ...
            <div v-if="info_usuario && consulta.estado === 'PENDIENTE' && info_usuario.tipo === 'veterinario'" class="item-actions">
    ...
    <div v-else class="no-auth">
      <p class="error">Inicia sesión para ver tus consultas.</p>
    </div>
    ...
    <div v-if="modalDetalleVisible" class="modal-overlay" @click="cerrarModalDetalle">
      <div class="modal-content modal-large" @click.stop>
        <div class="modal-header-detalle">
          ...
          <button class="close-x" @click="cerrarModalDetalle">&#10005;</button>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      mascotas: [],
      veterinarios: [],
      formCrear: { titulo: '', descripcion: '', mascota_id: '', vet_id: null },
      modalDetalleVisible: false,
      consultaSeleccionada: null,
      respuestasDetalle: [],
      cargandoRespuestas: false,
      modalResponderVisible: false,
      formResponder: { titulo: '', descripcion: '' }
    }
  },
  async created () { this.checkAuth() },
  methods: {
    checkAuth () {
      const token = localStorage.getItem('jwt'); const user = localStorage.getItem('user')
      if (token && user) {
        this.info_usuario = JSON.parse(user); this.jwtValido = true; this.obtenerConsultas()
      }
    },
    ...
    formatEstado (e) { return e === 'RESUELTA' ? 'RESUELTA' : 'PENDIENTE' },
    async abrirModalDetalle (c) {
      this.consultaSeleccionada = c; this.modalDetalleVisible = true
    ...
    async finalizarConsulta (id) {
      ...
      this.modalDetalleVisible = false; this.obtenerConsultas()
    },
    async abrirModalCrear () {
      this.modalVisible = true
      const resM = await api.get('http://localhost:5000/api/mascotas/listar-tus-mascotas'); this.mascotas = resM.data
      const resV = await api.get('http://localhost:5000/api/consultas/get-veterinarios'); this.veterinarios = resV.data
    },
    cerrarModalCrear () { this.modalVisible = false },
    async crearConsulta () {
      await api.post('http://localhost:5000/api/consultas/crear-consulta', this.formCrear)
      this.cerrarModalCrear(); this.obtenerConsultas()
    },
    abrirModalResponder (c) { this.consultaSeleccionada = c; this.modalResponderVisible = true },
    cerrarModalResponder () { this.modalResponderVisible = false },
    async enviarRespuesta () {
      await api.post(`http://localhost:5000/api/consultas/responder-consulta/${this.consultaSeleccionada.id}`, this.formResponder)
      this.cerrarModalResponder(); this.obtenerConsultas()
    },
    cerrarModalDetalle () { this.modalDetalleVisible = false }
  }
}
</script>
